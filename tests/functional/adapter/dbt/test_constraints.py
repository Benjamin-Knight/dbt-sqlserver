...
class BaseConstraintsRuntimeDdlEnforcement:
    ...
    def models(self):
        return {
            ...
            "foreign_key_model.sql": self.foreign_key_model_sql,
    ...
    def test__constraints_ddl(self, project, expected_sql):
        ...
        generated_sql_option_clause = generated_sql_wodb.replace(
            "OPTION (LABEL = ''dbt-sqlserver'');", ""
        )
        generated_sql_option_clause = generated_sql_option_clause.replace(
        ...
        assert _normalize_whitespace(expected_sql.strip()) == _normalize_whitespace(
            generated_sql_option_clause.strip()
        )
...
class BaseIncrementalConstraintsRuntimeDdlEnforcement(BaseConstraintsRuntimeDdlEnforcement):
    ...
    def models(self):
        return {
            "my_model.sql": self.my_model_incremental_wrong_order_depends_on_fk_sql,
            "foreign_key_model.sql": self.foreign_key_model_sql,
            "constraints_schema.yml": model_fk_constraint_schema_yml,
        }
