...
- Fix a model whose SQL references a column or object that does not exist upstream failing with `Attempt to use a closed connection` instead of the error naming what was wrong. A CTE-headed query has its shape read by `sp_describe_first_result_set` rather than by executing it, and that describe compiles the query - so the model fails *there*, with `Msg 207 Invalid column name`. Every describe failure was treated as the procedure declining to describe a good query and fell back to executing it, but handling the error has already closed the connection, so the fallback failed on that instead and the transport-level message replaced the real one, which survived only at debug level in `logs/dbt.log`. To anyone reading a scheduler's task log that looks like a network fault, and retrying cannot help because a missing column is not transient. The describe's own "metadata could not be determined" errors (the 11500-11599 family - a `#temp` table it cannot see through, a statement it cannot compile) still fall back to executing; anything else is now raised. Affects contract-enforced models and snapshot staging queries on every materialization.

#### Under the hood

- `get_column_schema_from_query` now reads a CTE-headed query's shape with `sp_describe_first_result_set` instead of executing it. Such queries cannot be wrapped as `select * from (...) where 1 = 0`, so they previously ran in full - a snapshot executed its whole staging query once for the probe and again to build the staging table, and a contract-enforced model ran twice per build. Describing compiles without scanning, so the cost no longer tracks data volume. Reported column names and types are unchanged: the describe path is mapped back onto the coarser names reading `cursor.description` produces, and falls back to executing whenever it cannot guarantee that (the `adbc` backend, a type outside the mapping, or a query `sp_describe_first_result_set` declines to describe, such as one reading a `#temp` table). The Python class behind a column is the driver's choice rather than SQL Server's, and the backends do not agree on all of them - `mssql-python` decodes `uniqueidentifier`, `datetimeoffset` and `sql_variant` into richer types than `pyodbc` does - so the mapping follows the backend in use, and `datetimeoffset` on `pyodbc` (whose class depends on when the `-155` output converter was registered) falls back to executing.
